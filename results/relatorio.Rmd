...
O p-valor foi de 0.7863, ou seja, não há evidências significativas de que o modelo completo seja melhor que o modelo reduzido. Então prosseguiremos a análise usando o modelo reduzido.
...
É possível observar a presença de algums valores anômalos;

```{r}
# dataframe com os dados originais + o d2 e o valor predito
dados_diagnostico <- data.frame(
  d2 = d_quadrado,
  probabilidade = predict(modelo_agrupado, type = "response"),
  model.frame(modelo_agrupado)
)

# Filtramos apenas os pontos acima da linha vermelha (3.84)
outliers_deviance <- dados_diagnostico[dados_diagnostico$d2 > 3.84, ]

# Ordenamos do maior erro para o menor
outliers_deviance <- outliers_deviance[order(-outliers_deviance$d2), ]

# Visualizar
pander::pander(outliers_deviance)
```

É possível observar casos em que os indivíduos carregam características de pessoas saudáveis, mas na verdade são doentes e vice versa; Entretanto essas observações são válidas, então não removeremos essas observações;

## Avaliação Final

Para avaliar a robustez do modelo final, foi realizado uma avaliação via Bootstrap Não Paramétrico com 1000 reamostragens. 

```{r}

tabela_final <- validar_modelo_bootstrap(
  modelo = modelo_agrupado, 
  dados_teste = teste_agrupado, 
  nome_coluna_alvo = "condition",
  R = 1000
)
```

```{r}
pander::pander(tabela_final)
```

A avaliação final via bootstrap confirma a estabilidade e robustez do modelo. Com uma acurácia média de 79,56% e métricas de sensibilidade e especificidade equilibradas (ambas acima de 77%), o modelo mostra-se capaz de generalizar o aprendizado para novos dados de forma confiável. O coeficiente Kappa de 58,39% ratifica que as previsões possuem consistência estatística, superando amplamente o fator acaso.